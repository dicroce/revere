        ...
        try
        {
            ...
            if(!_cameras.empty())
            {
                if(_ps.is_null())
                {
                    ...
                    auto blocks = _ws.get_blocks(ps.camera.id);
                ...
                else
                {
                    ...
                    auto end_ts = now - chrono::hours(current_ps.camera.min_continuous_recording_hours.value());

                    auto block_start = current_ps.blocks[current_ps.bi].start;
                    auto block_end = current_ps.blocks[current_ps.bi].end;

                    auto motion_events = _ws.get_motion_events(
