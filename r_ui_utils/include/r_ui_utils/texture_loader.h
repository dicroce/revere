...
public:
    ...
    std::pair<uint16_t, uint16_t> load_texture_from_image_memory(GLuint texture_id, const uint8_t* data, size_t size);
    std::pair<uint16_t, uint16_t> load_texture_from_image_file(GLuint texture_id, const std::string& filename);
    void load_texture_from_rgb_memory(GLuint texture_id, const uint8_t* data, size_t size, uint16_t width, uint16_t height);
