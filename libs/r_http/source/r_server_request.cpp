...
void r_server_request::read_request(r_utils::r_socket_base& socket)
{
...

    {
...

        {
            ...
            _read_header_line(socket, writer);
        }
        ...
        do
        {
            ...
            _read_header_line(socket, lineBuf);
...
void r_server_request::_read_header_line(r_socket_base& socket, char* writer)
{
    bool lineDone = false;
    ...
    {
        auto n_received = socket.recv(writer, 1);
        if( !socket.valid() )
            R_STHROW( r_http_io_exception, ("Socket invalid."));

        if(n_received > 0) {
            bytesReadThisLine += n_received;
...
void r_server_request::_process_body(r_socket_base& socket)
{
    ...
    if(!cl.is_null())
    {
        ...
        int received = r_networking::r_recv(socket, buffer, contentLength, timeout);
        if (received < (int)contentLength) {
            _body.resize(received);
        }
        if( !socket.valid() )
            R_STHROW( r_http_io_exception, ("Socket invalid."));
