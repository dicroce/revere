...
r_uri::r_uri( const string& fullURI ) :
...
r_uri r_uri::construct_uri( const string& resourcePath,
...
bool r_uri::operator == ( const r_uri& other ) const
{
...
string r_uri::get_full_resource_path() const
{
    return _resourcePath + _resource;
}

void r_uri::add_get_arg( const string& name, const string& value )
{
    if(name.empty())
        R_STHROW(r_http_exception_generic, ("Empty get argument name."));
    if(value.empty())
        R_STHROW(r_http_exception_generic, ("Empty get argument value."));
...
void r_uri::set_full_raw_uri(const string& fullURI)
{
...
void r_uri::set_get_args(const map<string, string>& getArgs)
{
    _fullRawURI = _construct_full_raw_uri(_resourcePath, _resource, getArgs);
    _getArgs = getArgs;
}
...
string r_uri::_construct_full_raw_uri(const string& resourcePath,
                                       const string& resource,
                                       const map<string, string>& getArgs)
{
...
map<string, string> r_uri::_parse_get_args(const string& fullURI)
{
...
string r_uri::_parse_resource(const string& fullRawURI)
{
...
string r_uri::_parse_resource_path(const string& fullRawURI)
{
...
string r_uri::_verify_starting_slash(const string& uri)
{
    if(uri.empty())
        return "/";
    else if(uri[0] != '/')
