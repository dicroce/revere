#ifndef __mobilenet_person_plugin_h
#define __mobilenet_person_plugin_h
...
class mobilenet_person_plugin : public r_vss::r_motion_plugin
{
public:
    struct Detection {
    ...
    R_API mobilenet_person_plugin(r_vss::r_motion_event_plugin_host* host);
    R_API virtual ~mobilenet_person_plugin();
    
    R_API virtual void post_motion_event(r_vss::r_motion_event evt, const std::string& camera_id, int64_t ts, const std::vector<uint8_t>& frame_data, uint16_t width, uint16_t height) override;
