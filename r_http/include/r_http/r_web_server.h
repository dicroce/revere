...
public:
    ...
    R_API r_web_server(int port, const std::string& sockAddr = std::string()) :
        _cbs(),
    ...
    R_API r_web_server(const r_web_server<SOK_T>&) = delete;

    R_API ~r_web_server() noexcept
    {
        stop();
    }

    R_API r_web_server<SOK_T>& operator = (const r_web_server<SOK_T>&) = delete;
    ...
    R_API void add_route( int method, const std::string& path, http_cb cb )
    {
        _cbs[method][path] = cb;
    }

    R_API SOK_T& get_socket() { return _server.get_socket(); }
